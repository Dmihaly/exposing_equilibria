...
function build_EPEC_quantity(t_steps, non_str_gens, non_str_res, str_agents, optimizer, SP_obj)
    ...
    function init_EPEC_quantity()
        ...
        #################FOR THE GENERATOR##########
            ...
            @NLconstraints(EPEC, begin
                ...
                #W.R.T. PRIMALS
                #w.r.t G_st{t}:
                [t in t_steps],  - κ⁻g[t] + κ⁺g[t] - ν⁺g_st[j,t] + σ_PD[j] * β⁺g_st[t] == 0
                #w.r.t dch_st{t}:
        ...
        #################FOR THE WIND##########
            ...
            @NLconstraints(EPEC, begin
                ...
                #W.R.T. PRIMALS
                #w.r.t W_st{t}:
                [t in t_steps],  - κ⁻w[t] + κ⁺w[t] - ν⁺w_st[j,t] + σ_PD[j] * β⁺w_st[t] == 0
                #w.r.t dch_st{t}:
                [t in t_steps], + c_dch_st[t]  - π[j,t] - ν⁻dch_st[j,t] + ν⁺dch_st[j,t] + σ_PD[j] * c_dch_st[t]  == 0
                #w.r.t ch_st{t}:
                [t in t_steps], - c_ch_st[t]  + π[j,t] - ν⁻ch_st[j,t] + ν⁺ch_st[j,t] - σ_PD[j] * c_ch_st[t] == 0
