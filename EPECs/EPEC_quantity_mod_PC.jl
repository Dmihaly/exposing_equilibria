...
function build_EPEC_quantity_mod_PC(t_steps, non_str_gens, non_str_res, str_agents, optimizer, SP_obj)
    ...
    ####INIT ABSTRACT MODEL######
    println("...Initializing EPEC quantity abstract model...")
    function init_EPEC_quantity()
        ...
        @variables(EPEC, begin
            ...
            0 <= γ⁺g_st[ t in t_steps]
            0 <= γ⁻g_st[ t in t_steps]
            0 <= γ⁺w_st[ t in t_steps]
            0 <= γ⁻w_st[ t in t_steps]
            ############UL DECISION VARS################
            ############PRICE BIDS###########
            c_dch_st[t in t_steps] #ESS price bid for discharging
            c_ch_st[t in t_steps] #ESS price bid for charging
            #########QUANTITY BIDS############
            ...
            ########SECOND STAGE DUALS#############
            0 <= ν⁺g[ i in non_str_gens, t in t_steps]
            0 <= ν⁻g[ i in non_str_gens, t in t_steps]
            0 <= ν⁺w[ k in non_str_res, t in t_steps]
            0 <= ν⁻w[ k in non_str_res, t in t_steps]
            0 <= ν⁺d[ t in t_steps]
            0 <= ν⁻d[ t in t_steps]
            0 <= ν⁺g_st[ t in t_steps]
            0 <= ν⁻g_st[ t in t_steps]
            0 <= ν⁺w_st[ t in t_steps]
            0 <= ν⁻w_st[ t in t_steps]
            0 <= ν⁺ch_st[ t in t_steps]
            0 <= ν⁻ch_st[ t in t_steps]
            0 <= ν⁺dch_st[ t in t_steps]
            0 <= ν⁻dch_st[ t in t_steps]
            ...
            ψg[ i in non_str_gens, t in t_steps]
            ψw[ k in non_str_res, t in t_steps]
            ψd[ t in t_steps]
            #For strategic agents:
            ψch_st[ t in t_steps]
            ψdch_st[ t in t_steps]
            ψg_st[ t in t_steps]
            ψw_st[ t in t_steps]
            0 <= δ⁺g[ i in non_str_gens, t in t_steps]
            0 <= δ⁻g[ i in non_str_gens, t in t_steps]
            0 <= δ⁺w[ k in non_str_res, t in t_steps]
            0 <= δ⁻w[ k in non_str_res, t in t_steps]
            0 <= δ⁺d[ t in t_steps]
            0 <= δ⁻d[ t in t_steps]
            0 <= δ⁺g_st[ t in t_steps]
            0 <= δ⁻g_st[ t in t_steps]
            0 <= δ⁺w_st[ t in t_steps]
            0 <= δ⁻w_st[ t in t_steps]
            0 <= δ⁺ch_st[ t in t_steps]
            0 <= δ⁻ch_st[ t in t_steps]
            0 <= δ⁺dch_st[ t in t_steps]
            0 <= δ⁻dch_st[ t in t_steps]
            ...
            0 <= κ⁺dch[t in t_steps] #Dual of the upper bound of the dch limit cstr
            0 <= κ⁻dch[t in t_steps] #Dual of the lower bound of the dch limit cstr
            0 <= κ⁺ch[t in t_steps] #Dual of the upper bound of the ch limit cstr
            0 <= κ⁻ch[t in t_steps] #Dual of the lower bound of the ch limit cstr
            #FOR THE GEN
            0 <= κ⁺g[t in t_steps] #Dual of the upper bound of the gen limit cstr
            0 <= κ⁻g[t in t_steps] #Dual of the lower bound of the gen limit cstr
            #FOR THE GEN
            0 <= κ⁺w[t in t_steps] #Dual of the upper bound of the gen limit cstr
            0 <= κ⁻w[t in t_steps] #Dual of the lower bound of the gen limit cstr
        ...
        @constraints(EPEC, begin
            ...
            #SOC RULES
            #NOTE: SOC RULES ARE NOT COMMON TO ALL AGENT BUT THERE IS NO DIFFERENTIATION IN J VARS
        ...
        #################FOR THE ESS##########
            ...
            @NLconstraints(EPEC, begin
                ...
                #W.R.T. PRIMALS
                #w.r.t c_dch_st{t}:
                [t in t_steps], - ϵ⁻dch[t] + ϵ⁺dch[t] + σ_PD[j] * dch_st[t] + ψdch_st[t] == 0
                #w.r.t c_ch_st{t}:
                [t in t_steps], - ϵ⁻ch[t] + ϵ⁺ch[t] - σ_PD[j] * ch_st[t] - ψch_st[t] == 0
                #w.r.t DCH_st{t}:
                [t in t_steps],  - κ⁻dch[t] + κ⁺dch[t] - ν⁺dch_st[t] - γ⁺dch_st[t] * β⁺dch_st[t] + γ⁻dch_st[t] * β⁺dch_st[t]  == 0
                #w.r.t CH_st{t}:
                [t in t_steps],  - κ⁻ch[t] + κ⁺ch[t] - ν⁺ch_st[t] - γ⁺ch_st[t] * β⁺ch_st[t] + γ⁻ch_st[t] * β⁺ch_st[t] == 0
                #w.r.t dch_st{t}:
                [t in t_steps],  - π[t] - ν⁻dch_st[t] + ν⁺dch_st[t] + σ_PD[j] * c_dch_st[t] - (μsoc[t]/ηdch)  == 0
                #w.r.t ch_st{t}:
                [t in t_steps],  + π[t] - ν⁻ch_st[t] + ν⁺ch_st[t] - σ_PD[j] * c_ch_st[t] + (μsoc[t]*ηch) == 0
                #w.r.t g_st{t}:
                [t in t_steps], + c_g_st[t] - π[t] - ν⁻g_st[t] + ν⁺g_st[t] + σ_PD[j] * c_g_st[t] == 0
                #w.r.t g{t}:
                [i in non_str_gens, t in t_steps], + c_g[i,t] - π[t] - ν⁻g[i,t] + ν⁺g[i,t] + σ_PD[j] * c_g[i,t] == 0
                #w.r.t w_st{t}:
                [t in t_steps], + c_w_st[t] - π[t] - ν⁻w_st[t] + ν⁺w_st[t] + σ_PD[j] * c_w_st[t] == 0
                #w.r.t w{t}:
                [k in non_str_res, t in t_steps], + c_w[k,t] - π[t] - ν⁻w[k,t] + ν⁺w[k,t] + σ_PD[j] * c_w[k,t] == 0
                #w.r.t d{t}:
                [t in t_steps], - c_d[t] + π[t] - ν⁻d[t] + ν⁺d[t] - σ_PD[j] * c_d[t] == 0
                #w.r.t SOC{t}:
                [t = t_steps[end]], + μsoc[1] - μsoc[t] - κ⁻soc[t] + κ⁺soc[t] + μ_0 == 0
                [t in 1:t_steps[end-1]], + μsoc[t+1] - μsoc[t] - κ⁻soc[t] + κ⁺soc[t] == 0
                #W.R.T. DUALS
                #w.r.t λ{t}:
                [t in t_steps],  + ψd[t] - ψg_st[t]  - ψw_st[t] - sum(ψg[i,t] for i in non_str_gens) - sum(ψw[k,t] for k in non_str_res) + ψch_st[t] - ψdch_st[t] == 0
                #w.r.t β⁺g_st{t}:
                [t in t_steps], - γ⁺g_st[t] * G_ST[t] + γ⁻g_st[t] * G_ST[t] + ψg_st[t] - δ⁺g_st[t] == 0
                #w.r.t β⁻g_st{t}:
                [t in t_steps], - ψg_st[t] - δ⁻g_st[t] == 0
                #w.r.t β⁺g{i,t}:
                [i in non_str_gens, t in t_steps], + G[i,t] + σ_PD[j] * G[i,t] + ψg[i,t] - δ⁺g[i,t] == 0
                #w.r.t β⁻g{i,t}:
                [i in non_str_gens, t in t_steps], - ψg[i,t] - δ⁻g[i,t] == 0
                #w.r.t β⁺w_st{t}:
                [t in t_steps], - γ⁺w_st[t] * W_ST[t] + γ⁻w_st[t] * W_ST[t] + ψw_st[t] - δ⁺w_st[t] == 0
                #w.r.t β⁻w_st{t}:
                [t in t_steps], - ψw_st[t] - δ⁻w_st[t] == 0
                #w.r.t β⁺w{k,t}:
                [k in non_str_res, t in t_steps], + W[k,t] + σ_PD[j] * W[k,t] + ψw[k,t] - δ⁺w[k,t] == 0
                #w.r.t β⁻w{k,t}:
                [k in non_str_res, t in t_steps], - ψw[k,t] - δ⁻w[k,t] == 0
                #w.r.t β⁺dch_st{t}:
                [t in t_steps], - γ⁺dch_st[t] * DCH_ST[t] + γ⁻dch_st[t] * DCH_ST[t] + ψdch_st[t] - δ⁺dch_st[t] == 0
                #w.r.t β⁻dch_st{t}:
                [t in t_steps], - ψdch_st[t] - δ⁻dch_st[t] == 0
                #w.r.t β⁺ch_st{t}:
                [t in t_steps], - γ⁺ch_st[t] * CH_ST[t] + γ⁻ch_st[t] * CH_ST[t] + ψch_st[t] - δ⁺ch_st[t] == 0
                #w.r.t β⁻ch_st{t}:
                [t in t_steps], - ψch_st[t] - δ⁻ch_st[t] == 0
                #w.r.t β⁺d{t}:
                [t in t_steps], + D[t] + σ_PD[j] * D[t] + ψd[t] - δ⁺d[t] == 0
                #w.r.t β⁻d{t}:
                [t in t_steps], - ψd[t] - δ⁻d[t] == 0

                #w.r.t. s_dch:
                [t in t_steps],  + γ⁺dch_st[t] - γ⁻dch_st[t] + σ_PD[j] == 0
                #w.r.t. s_ch:
                [t in t_steps],  + γ⁺ch_st[t] - γ⁻ch_st[t] + σ_PD[j] == 0
                #w.r.t. s_g:
                [t in t_steps],  + γ⁺g_st[t] - γ⁻g_st[t] + σ_PD[j] == 0
                #w.r.t. s_w:
                [t in t_steps],  + γ⁺w_st[t] - γ⁻w_st[t] + σ_PD[j] == 0
            ...
            @NLconstraints(EPEC, begin
                ...
                [t in t_steps], -β⁺w_st[t] * W_ST[t] + r_w_st[t] <= ϵ1
                #CS PART
                [t in t_steps], (ϵ1 - (β⁺ch_st[t] * CH_ST[t] - r_ch_st[t]) * γ⁻ch_st[t]) <= ϵ2
                [t in t_steps], (ϵ1 - (-β⁺ch_st[t] * CH_ST[t] + r_ch_st[t]) * γ⁺ch_st[t]) <= ϵ2
                [t in t_steps], (ϵ1 - (β⁺dch_st[t] * DCH_ST[t] - r_dch_st[t]) * γ⁻dch_st[t]) <= ϵ2
                [t in t_steps], (ϵ1 - (-β⁺dch_st[t] * DCH_ST[t] + r_dch_st[t]) * γ⁺dch_st[t]) <= ϵ2
                [t in t_steps], (ϵ1 - (β⁺g_st[t] * G_ST[t] - r_g_st[t]) * γ⁻g_st[t]) <= ϵ2
                [t in t_steps], (ϵ1 - (-β⁺g_st[t] * G_ST[t] + r_g_st[t]) * γ⁺g_st[t]) <= ϵ2
                [t in t_steps], (ϵ1 - (β⁺w_st[t] * W_ST[t] - r_w_st[t]) * γ⁻w_st[t]) <= ϵ2
                [t in t_steps], (ϵ1 - (-β⁺w_st[t] * W_ST[t] + r_w_st[t]) * γ⁺w_st[t]) <= ϵ2

                #LL related
                [i in non_str_gens, t in t_steps], g[i,t] * ν⁻g[i,t]  <= ϵ2
                [i in non_str_gens, t in t_steps], s_g[i,t] * ν⁺g[i,t]  <= ϵ2
                [i in non_str_gens, t in t_steps], β⁻g[i,t] *  δ⁻g[i,t]  <= ϵ2
                [i in non_str_gens, t in t_steps], β⁺g[i,t] *  δ⁺g[i,t] <= ϵ2
                [k in non_str_res, t in t_steps], w[k,t] * ν⁻w[k,t]  <= ϵ2
                [k in non_str_res, t in t_steps], s_w[k,t] * ν⁺w[k,t]  <= ϵ2
                [k in non_str_res, t in t_steps], β⁻w[k,t] *  δ⁻w[k,t]  <= ϵ2
                [k in non_str_res, t in t_steps], β⁺w[k,t] *  δ⁺w[k,t] <= ϵ2
                [t in t_steps], g_st[t] * ν⁻g_st[t]  <= ϵ2
                [t in t_steps], s_g_st[t] * ν⁺g_st[t]  <= ϵ2
                [t in t_steps], β⁻g_st[t] *  δ⁻g_st[t]  <= ϵ2
                [t in t_steps], β⁺g_st[t] *  δ⁺g_st[t] <= ϵ2
                [t in t_steps], w_st[t] * ν⁻w_st[t]  <= ϵ2
                [t in t_steps], s_w_st[t] * ν⁺w_st[t]  <= ϵ2
                [t in t_steps], β⁻w_st[t] *  δ⁻w_st[t]  <= ϵ2
                [t in t_steps], β⁺w_st[t] *  δ⁺w_st[t] <= ϵ2
                [t in t_steps], dch_st[t] * ν⁻dch_st[t]  <= ϵ2
                [t in t_steps], s_dch_st[t] * ν⁺dch_st[t]  <= ϵ2
                [t in t_steps], β⁻dch_st[t] *  δ⁻dch_st[t]  <= ϵ2
                [t in t_steps], β⁺dch_st[t] *  δ⁺dch_st[t] <= ϵ2
                [t in t_steps], ch_st[t] * ν⁻ch_st[t]  <= ϵ2
                [t in t_steps], s_ch_st[t] * ν⁺ch_st[t]  <= ϵ2
                [t in t_steps], β⁻ch_st[t] *  δ⁻ch_st[t]  <= ϵ2
                [t in t_steps], β⁺ch_st[t] *  δ⁺ch_st[t] <= ϵ2
                [t in t_steps], d[t] * ν⁻d[t]  <= ϵ2
                [t in t_steps], s_d[t] * ν⁺d[t]  <= ϵ2
                [t in t_steps], β⁻d[t] *  δ⁻d[t]  <= ϵ2
                [t in t_steps], β⁺d[t] *  δ⁺d[t] <= ϵ2
                #UL related
                #Quantity related
                [t in t_steps], t_ch_bid[t] * κ⁺ch[t]  <= ϵ2
                [t in t_steps], DCH_ST[t] * κ⁻ch[t]  <= ϵ2
                [t in t_steps], t_dch_bid[t] * κ⁺dch[t]  <= ϵ2
                [t in t_steps], CH_ST[t] * κ⁻dch[t]  <= ϵ2
                [t in t_steps], t_soc[t] * κ⁺soc[t]  <= ϵ2
                [t in t_steps], SOC[t] * κ⁻soc[t] <= ϵ2
                #Price related
        ...
        #################FOR THE GENERATOR##########
            ...
            @NLconstraints(EPEC, begin
                ...
                #W.R.T. PRIMALS
                #w.r.t G_st{t}:
                [t in t_steps],  - κ⁻g[t] + κ⁺g[t] - ν⁺g_st[t] - γ⁺g_st[t] * β⁺g_st[t] + γ⁻g_st[t] * β⁺g_st[t] == 0
                #w.r.t dch_st{t}:
                [t in t_steps], + c_dch_st[t] - π[t] - ν⁻dch_st[t] + ν⁺dch_st[t] + σ_PD[j] * c_dch_st[t]  == 0
                #w.r.t ch_st{t}:
                [t in t_steps], - c_ch_st[t]  + π[t] - ν⁻ch_st[t] + ν⁺ch_st[t] - σ_PD[j] * c_ch_st[t] == 0
                #w.r.t g_st{t}:
                [t in t_steps], + dat_str.GEN_STR.MC_g[t] - π[t] - ν⁻g_st[t] + ν⁺g_st[t] + σ_PD[j] * c_g_st[t] == 0
                #w.r.t g{t}:
                [i in non_str_gens, t in t_steps], + c_g[i,t] - π[t] - ν⁻g[i,t] + ν⁺g[i,t] + σ_PD[j] * c_g[i,t] == 0
                #w.r.t w_st{t}:
                [t in t_steps], + c_w_st[t] - π[t] - ν⁻w_st[t] + ν⁺w_st[t] + σ_PD[j] * c_w_st[t] == 0
            ...
            @NLconstraints(EPEC, begin
                ...
                #UL related
                #Quantity related
                [t in t_steps], t_g_bid[t] * κ⁺g[t]  <= ϵ2
                [t in t_steps], G_ST[t] * κ⁻g[t]  <= ϵ2
            end)
        ...
        #################FOR THE WIND##########
            ...
            @NLconstraints(EPEC, begin
                ...
                #W.R.T. PRIMALS
                #w.r.t W_st{t}:
                [t in t_steps],  - κ⁻w[t] + κ⁺w[t] - ν⁺w_st[t] - γ⁺w_st[t] * β⁺w_st[t] + γ⁻w_st[t] * β⁺w_st[t] == 0
                #w.r.t dch_st{t}:
                [t in t_steps], + c_dch_st[t]  - π[t] - ν⁻dch_st[t] + ν⁺dch_st[t] + σ_PD[j] * c_dch_st[t]  == 0
                #w.r.t ch_st{t}:
                [t in t_steps], - c_ch_st[t]  + π[t] - ν⁻ch_st[t] + ν⁺ch_st[t] - σ_PD[j] * c_ch_st[t] == 0
                ...
                [i in non_str_gens, t in t_steps], + c_g[i,t] - π[t] - ν⁻g[i,t] + ν⁺g[i,t] + σ_PD[j] * c_g[i,t] == 0
                #w.r.t w_st{t}:
                [t in t_steps], + dat_str.WIND_STR.MC_g[t] - π[t] - ν⁻w_st[t] + ν⁺w_st[t] + σ_PD[j] * c_w_st[t] == 0
                #w.r.t w{t}:
                [k in non_str_res, t in t_steps], + c_w[k,t] - π[t] - ν⁻w[k,t] + ν⁺w[k,t] + σ_PD[j] * c_w[k,t] == 0
                #w.r.t d{t}:
                [t in t_steps], - c_d[t] + π[t] - ν⁻d[t] + ν⁺d[t] - σ_PD[j] * c_d[t] == 0
            ...
            @NLconstraints(EPEC, begin
                ...
                #UL related
                #Quantity related
                [t in t_steps], t_w_bid[t] * κ⁺w[t]  <= ϵ2
                [t in t_steps], W_ST[t] * κ⁻w[t]  <= ϵ2
            end)

        return EPEC
    end

    EPEC = init_EPEC_quantity()
    ...
    function get_EPEC_obj()
        ...
        elseif SP_obj == "collusive"
            ...
            #     - (
            #     # - sum(EPEC[:g_st][t] * dat_str.GEN_STR.MC_g[t] for t in t_steps) #GEN_STR
            #     - sum(EPEC[:dch_st][t] * dat_str.ESS_STR.c_dch[t] for t in t_steps) #DCH_STR
            #     + sum(EPEC[:ch_st][t] * dat_str.ESS_STR.c_ch[t] for t in t_steps) #DCH_STR
            #     - sum(EPEC[:w_st][t] * dat_str.WIND_STR.MC_g[t] for t in t_steps) #WIND_STR
            #     - sum(sum(EPEC[:g][i,t] * dat_nonstr[i].c_g[t] for t in t_steps) for i in non_str_gens) #GENs
            #     - sum(sum(EPEC[:w][k,t] * dat_nonstr[k].c_g[t] for t in t_steps) for k in non_str_res) #GENs
            #     + sum(EPEC[:d][t] .* p_cap for t in t_steps)
