"This module is created to build the SCR-NLP version of the MPPDC with quanity bids, with primal objective function, for all UL agents,
Definitation of which UL problem that is being built is based on the 'current' string passed in"
function build_NLP_primal_quantity(t_steps, non_str_gens, non_str_res, str_agents, current, optimizer)
    ...
    if current == "ESS_STR"
        ...
        function init_NLP_ESS()
            ...
            @NLconstraints(NLP, begin
                ...
                #w.r.t dch_st{t}:
                [t in t_steps], - λ[t] - π[j,t] - ν⁻dch_st[j,t] + ν⁺dch_st[j,t] + σ_PD[j] * c_dch_st[t] - (μsoc[t]/ηdch)  == 0
                #w.r.t ch_st{t}:
                [t in t_steps], + λ[t] + π[j,t] - ν⁻ch_st[j,t] + ν⁺ch_st[j,t] - σ_PD[j] * c_ch_st[t] + (μsoc[t]*ηch) == 0
                #w.r.t g_st{t}:
                [t in t_steps], - π[j,t] - ν⁻g_st[j,t] + ν⁺g_st[j,t] + σ_PD[j] * c_g_st[t] == 0
                #w.r.t g{t}:
                [i in non_str_gens, t in t_steps], - π[j,t] - ν⁻g[j,i,t] + ν⁺g[j,i,t] + σ_PD[j] * c_g[i,t] == 0
                #w.r.t w_st{t}:
                [t in t_steps], - π[j,t] - ν⁻w_st[j,t] + ν⁺w_st[j,t] + σ_PD[j] * c_w_st[t] == 0
                #w.r.t w{t}:
                [k in non_str_res, t in t_steps], - π[j,t] - ν⁻w[j,k,t] + ν⁺w[j,k,t] + σ_PD[j] * c_w[k,t] == 0
                #w.r.t d{t}:
                [t in t_steps], + π[j,t] - ν⁻d[j,t] + ν⁺d[j,t] - σ_PD[j] * c_d[t] == 0
                #w.r.t SOC{t}:
                [t = t_steps[end]], + μsoc[1] - μsoc[t] - κ⁻soc[t] + κ⁺soc[t] + μ_0 == 0
                [t in 1:t_steps[end-1]], + μsoc[t+1] - μsoc[t] - κ⁻soc[t] + κ⁺soc[t] == 0
                #W.R.T. DUALS
                #w.r.t λ{t}:
                [t in t_steps], ch_st[t] - dch_st[t] + ψd[j,t] - ψg_st[j,t]  - ψw_st[j,t] - sum(ψg[j,i,t] for i in non_str_gens) - sum(ψw[j,k,t] for k in non_str_res) + ψch_st[j,t] - ψdch_st[j,t] == 0
                #w.r.t β⁺g_st{t}:
                [t in t_steps], σ_PD[j] * G_ST[t] + ψg_st[j,t] - δ⁺g_st[j,t] == 0
                #w.r.t β⁻g_st{t}:
                [t in t_steps], - ψg_st[j,t] - δ⁻g_st[j,t] == 0
                #w.r.t β⁺g{i,t}:
                [i in non_str_gens, t in t_steps], σ_PD[j] * G[i,t] + ψg[j,i,t] - δ⁺g[j,i,t] == 0
                #w.r.t β⁻g{i,t}:
                ...
                #w.r.t β⁺w{k,t}:
                [k in non_str_res, t in t_steps], σ_PD[j] * W[k,t] + ψw[j,k,t] - δ⁺w[j,k,t] == 0
                #w.r.t β⁻w{k,t}:
                ...
                #w.r.t β⁺d{t}:
                [t in t_steps], σ_PD[j] * D[t] + ψd[j,t] - δ⁺d[j,t] == 0
                #w.r.t β⁻d{t}:
    ...
    elseif current == "GEN_STR"
        ...
        function init_NLP_GEN()
            ...
            param[:c_g_st] = @NLparameter(NLP, c_g_st[t in t_steps] == 0) #Generation price bid str
            param[:c_w_st] = @NLparameter(NLP, c_w_st[t in t_steps]  == 0)#Wind price bid str
            param[:c_dch_st] = @NLparameter(NLP, c_dch_st[t in t_steps] == 0) #ESS discharge price bid str
            param[:c_ch_st] = @NLparameter(NLP, c_ch_st[t in t_steps] == 0) #ESS charge price bid str
            param[:c_g] = @NLparameter(NLP, c_g[i in non_str_gens, t in t_steps] == 0) #Generation price bid nonstr
            param[:c_w] = @NLparameter(NLP, c_w[k in non_str_res, t in t_steps] == 0) #Wind price bid nonstr
            param[:c_d] = @NLparameter(NLP, c_d[t in t_steps] == 0) #WTP of the load
            ...
            @NLconstraints(NLP, begin
                ...
                #w.r.t dch_st{t}:
                [t in t_steps],  - π[j,t] - ν⁻dch_st[j,t] + ν⁺dch_st[j,t] + σ_PD[j] * c_dch_st[t]  == 0
                #w.r.t ch_st{t}:
                [t in t_steps],  + π[j,t] - ν⁻ch_st[j,t] + ν⁺ch_st[j,t] - σ_PD[j] * c_ch_st[t] == 0
                #w.r.t g_st{t}:
                [t in t_steps], + dat_str.GEN_STR.MC_g[t] - λ[t]  - π[j,t] - ν⁻g_st[j,t] + ν⁺g_st[j,t] + σ_PD[j] * c_g_st[t] == 0
                #w.r.t g{t}:
                [i in non_str_gens, t in t_steps], - π[j,t] - ν⁻g[j,i,t] + ν⁺g[j,i,t] + σ_PD[j] * c_g[i,t] == 0
                #w.r.t w_st{t}:
                [t in t_steps], - π[j,t] - ν⁻w_st[j,t] + ν⁺w_st[j,t] + σ_PD[j] * c_w_st[t] == 0
                ...
                #W.R.T. DUALS
                #w.r.t λ{t}:
                [t in t_steps], - g_st[t] + ψd[j,t] - ψg_st[j,t]  - ψw_st[j,t] - sum(ψg[j,i,t] for i in non_str_gens) - sum(ψw[j,k,t] for k in non_str_res) + ψch_st[j,t] - ψdch_st[j,t] == 0
                #w.r.t β⁺g_st{t}:
                [t in t_steps], σ_PD[j] * G_ST[t] + ψg_st[j,t] - δ⁺g_st[j,t] == 0
                #w.r.t β⁻g_st{t}:
                [t in t_steps], - ψg_st[j,t] - δ⁻g_st[j,t] == 0
    ...
    elseif current == "WIND_STR"
        ...
        function init_NLP_RES()
            ...
            @NLconstraints(NLP, begin
                ...
                [i in non_str_gens, t in t_steps], - π[j,t] - ν⁻g[j,i,t] + ν⁺g[j,i,t] + σ_PD[j] * c_g[i,t] == 0
                #w.r.t w_st{t}:
                [t in t_steps], + dat_str.WIND_STR.MC_g[t] - λ[t] - π[j,t] - ν⁻w_st[j,t] + ν⁺w_st[j,t] + σ_PD[j] * c_w_st[t] == 0
                #w.r.t w{t}:
                [k in non_str_res, t in t_steps], - π[j,t] - ν⁻w[j,k,t] + ν⁺w[j,k,t] + σ_PD[j] * c_w[k,t] == 0
                #w.r.t d{t}:
                [t in t_steps], + π[j,t] - ν⁻d[j,t] + ν⁺d[j,t] - σ_PD[j] * c_d[t] == 0
                ...
                #W.R.T. DUALS
                #w.r.t λ{t}:
                [t in t_steps], - w_st[t] + ψd[j,t] - ψg_st[j,t]  - ψw_st[j,t] - sum(ψg[j,i,t] for i in non_str_gens) - sum(ψw[j,k,t] for k in non_str_res) + ψch_st[j,t] - ψdch_st[j,t] == 0
                #w.r.t β⁺g_st{t}:
                [t in t_steps], σ_PD[j] * G_ST[t] + ψg_st[j,t] - δ⁺g_st[j,t] == 0
                #w.r.t β⁻g_st{t}:
                [t in t_steps], - ψg_st[j,t] - δ⁻g_st[j,t] == 0
