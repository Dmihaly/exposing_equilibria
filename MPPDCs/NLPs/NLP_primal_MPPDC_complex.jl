...
function build_NLP_primal_complex(t_steps, non_str_gens, non_str_res, str_agents, current, optimizer)
    ...
    if current == "ESS_STR"
        ...
        function init_NLP_ESS()
            ...
            @NLconstraints(NLP, begin
                ...
                #w.r.t dch_st{t}:
                [t in t_steps], - λ[t] - π[j,t] - ν⁻dch_st[j,t] + ν⁺dch_st[j,t]  - (Μsoc[j,t]/ηdch)  == 0
                #w.r.t ch_st{t}:
                [t in t_steps], + λ[t] + π[j,t] - ν⁻ch_st[j,t] + ν⁺ch_st[j,t] + (Μsoc[j,t]*ηch) == 0
                #w.r.t. SoC{l,t}
                ...
                #w.r.t κ⁺soc{t}:
                [t in t_steps], σ_PD[j] * E_max[t] + ψsoc[j,t] - δ⁺soc[j,t] == 0
                #w.r.t κ⁻soc{t}:
