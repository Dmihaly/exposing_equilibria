"This module is created to build MPEC with complex bids, for all UL agents,
Definitation of which UL problem is being built is based on the 'current' string passed in"
function build_MPEC_complex(t_steps, non_str_gens, non_str_res, current, optimizer, objective_type)
    ...
    if current == "ESS_STR"
        ...
        function init_MPEC_ESS()
            ...
            @NLconstraints(MPPDC, begin
                ...
                #w.r.t g_{i,t}:
                [i in non_str_gens, t in t_steps], g[i,t] * β⁻g[i,t] <= ϵ
                [i in non_str_gens, t in t_steps], (G[i,t] - g[i,t]) * β⁺g[i,t] <= ϵ
                #w.r.t g_st_{i,t}:
                [t in t_steps], g_st[t] * β⁻g_st[t] <= ϵ
                [t in t_steps], (G_ST[t] - g_st[t]) * β⁺g_st[t] <= ϵ
                #w.r.t w_{k,t}:
                [k in non_str_res, t in t_steps], w[k,t] * β⁻w[k,t] <= ϵ
                [k in non_str_res, t in t_steps], (W[k,t] - w[k,t]) * β⁺w[k,t] <= ϵ
                #w.r.t w_st_{t}:
                [t in t_steps], w_st[t] * β⁻w_st[t] <= ϵ
                [t in t_steps], (W_ST[t] - w_st[t]) * β⁺w_st[t] <= ϵ
                #w.r.t dch_st_{t}:
                [t in t_steps], dch_st[t] * β⁻dch_st[t] <= ϵ
                [t in t_steps], (DCH_ST[t] - dch_st[t]) * β⁺dch_st[t] <= ϵ
                #w.r.t ch_st_{t}:
                [t in t_steps], ch_st[t] * β⁻ch_st[t] <= ϵ
                [t in t_steps], (CH_ST[t] - ch_st[t]) * β⁺ch_st[t] <= ϵ
                #w.r.t d_{t}:
                [t in t_steps], d[t] * β⁻d[t] <= ϵ
                [t in t_steps], (D[t] - d[t]) * β⁺d[t] <= ϵ
                #w.r.t SOC_{t}:
                [t in t_steps], SOC[t] * κ⁻soc[t] <= ϵ
                [t in t_steps], (E_max[t] - SOC[t]) * κ⁺soc[t] <= ϵ
