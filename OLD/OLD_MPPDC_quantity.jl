"This module is created to build MPPDC with quanity bids, for all UL agents,
Definitation of which UL problem is being built is based on the 'current' string passed in"
function build_MPPDC_quantity(t_steps, non_str_gens, non_str_res, current, optimizer)
    ...
    function fix_other_agent_vars()
        ...
        if current == "ESS_STR"
            for t in t_steps
                #DELETING THE EXCLUSIVENESS CSTR
                # JuMP.delete(MPPDC, MPPDC[:excl][t])
